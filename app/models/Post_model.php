...
class Post_model
{
    ...
    public function get_posts_count($filter) 
    {
...
// получение постов теущей страницы
    public function get_page_posts($offset, $size_page, $filter) {
        if ($filter == 'all') {
    ...
    public function create($form_data, $img) 
    {
        $img_path = $this->save_img($img);
    ...
    public function update($form_data, $img, $id) 
    {
        $img_path = $this->save_img($img);
    ...
    protected function save_img($img) 
